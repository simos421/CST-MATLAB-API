...
function Cstcreatedielectricmaterial(mws, Name, Epsilon, Mue, TanD, ThermalConductivity)

material = invoke(mws,'material');
invoke(material,'Reset');
invoke(material,'Name',Name); % Rogers TMM 10i (lossy)
invoke(material,'FrqType','all');
invoke(material,'Type','Normal');
invoke(material,'SetMaterialUnit','GHz','mm');
invoke(material,'Epsilon',int2str(Epsilon));
invoke(material,'Mue',int2str(Mue));
invoke(material,'Kappa','0.0');
invoke(material,'TanD',int2str(TanD));
...
invoke(material,'ThermalConductivity',int2str(ThermalConductivity));
invoke(material,'ThermalType','Normal'); 
